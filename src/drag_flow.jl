...
"""
function solve_drag_flow(model::DiscreteModel, top_velocity::Real, rheology, temperature::Real; order=1, quad_order=10)
...
"""
function write_drag_flow_vtk(uh, model::DiscreteModel, rheology, temperature::Real, filename::String; quad_order=10)
    Ω = Triangulation(model)
    grad_uh = ∇(uh)
    ...
    writevtk(Ω, filename, cellfields=[
        "u" => uh,
        "shear_rate" => γₕ,
        "shear_stress" => τₕ,
        "viscosity" => μₕ
...
"""
function simulate_drag_flow(msh_file::String, top_velocity::Real, rheology, temperature::Real; order=1, quad_order=10)
    model = GmshDiscreteModel(msh_file)

    println("Solving drag flow for $msh_file...")
    uh, a, v_space, dirichlet_tags = solve_drag_flow(model, top_velocity, rheology, temperature; order=order, quad_order=quad_order)
    ...
    write_drag_flow_vtk(uh, model, rheology, temperature, vtu_name; quad_order=quad_order)
